...
function AppContent() {
  ...
  const handleSubmit = (event) => {
    ...
    if (unitSystem === "imperial") {
      weightValue = convertWeight(weight, "lbs", "kg")
    }
  ...
  }, []) // Removed dependency on fitnessEntries as it's not directly used here
  ...
  const handleDownloadCsv = () => {
    ...
    fitnessEntries.forEach((entry) => {
      ...
      const workoutSplitCsv = `"${(Array.isArray(entry.workoutSplit) ? entry.workoutSplit : [entry.workoutSplit])
        .map((s) => s.replace(/"/g, '""'))
        .join(", ")}"`
  ...
  const handleSupplementCheckboxChange = (name, checked) => {
    setFormSupplements((prev) => prev.map((sup) => (sup.name === name ? { ...sup, taken: checked } : sup)))
  }

  const handleSupplementQuantityChange = (name, quantity) => {
    setFormSupplements((prev) => prev.map((sup) => (sup.name === name ? { ...sup, quantity: quantity } : sup)))
  }
    ...
    <>
      ...
      >
                  ...
                  <Grid container spacing={2}>
                    <Grid item xs={12}>
                      <FormControl fullWidth required sx={{ minWidth: 250 }}>
                        ...
                        >
                          <MenuItem value="">Select Workout(s)</MenuItem> {/* Added option for initial clear state */}
        ...
        {currentTab === 1 && (
          <Box sx={{ mt: 2, p: 4 }} component={Paper} elevation={0}>
            ...
            <Grid container spacing={2} sx={{ mt: 2 }}>
              ...
              <Grid item xs={12} sm={6} md={4}>
                <Typography variant="body2">
                  <strong>Most Frequent Workout:</strong> {stats.mostFrequentWorkout}
                </Typography>
              </Grid>
              <Grid item xs={12} sm={6} md={4}>
                <Typography variant="body2">
              ...
              <ResponsiveContainer width="100%" height={300}>
                <LineChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                  ...
                  <YAxis
                    yAxisId="left"
                    label={{
                      value: `Weight (${unitSystem === "metric" ? "kg" : "lbs"})`,
                      angle: -90,
                      position: "insideLeft",
        ...
        >
          ...
          <DialogContent>
            <DialogContentText id="delete-custom-supplement-dialog-description">
              Are you sure you want to remove "{supplementToDelete}" from your custom supplements? This will not remove
              it from existing entries.
            </DialogContentText>
          </DialogContent>
          <DialogActions>
        ...
        >
          ...
          <DialogContent>
            <DialogContentText id="simple-alert-dialog-description">{simpleAlertDialogMessage}</DialogContentText>
          </DialogContent>
          <DialogActions>
