...
function ExercisePill({ ex, onRemove }) {
  ...
  const isCardioEx = ex.category === "cardio" && (ex.cardioType === "activity" || ex.cardioType === "machine")
...
export default function WorkoutPlanner({ pendingExercise, onPendingConsumed }) {
  ...
  const confirmAddExercise = () => {
    ...
    const isCardioEx = pendingExercise.category === "cardio" && (pendingExercise.cardioType === "activity" || pendingExercise.cardioType === "machine")
    const entry = {
        ...
        <DialogContent sx={{ display: "flex", flexDirection: "column", gap: 2, pt: 2 }}>
          ...
          {plans.length === 0 ? (
            ...
            <>
              ...
              {(pendingExercise?.cardioType === "activity" || pendingExercise?.cardioType === "machine") ? (
                /* ── Cardio: rounds only ── */
                <TextField
